...
The \textit{mRMRe} package is for this and that

%------------------------------------------------------------
\subsection{Installation}
%------------------------------------------------------------

\textit{mRMRe} requires that \textit{Rcpp} is installed. These should be installed automatically when you install \textit{mRMRe}. To install \textit{mRMRe}, source biocLite from Bioconductor:
...
<<eval=FALSE>>=
merged_data <- data.frame(ic50=cgps_ic50, cgps_ge)

# Pearson correlation for continuous variables
mim.pearson <- build.mim(data=merged_data, uses_ranks=FALSE)

# Spearman correlation for continuous variables
mim.spearman <- build.mim(data=merged_data) 
@
