...
%------------------------------------------------------------
mRMRe offers a fully parallelized implementation of the Mutual Information Matrix.
The method supports the following column types: "numeric", "ordered_factor" and "Surv".
...
%------------------------------------------------------------
The mRMRe package offers an efficient, stratified and weighted implementation of the major correlation estimators
(Cramer's V, C-Index, Pearson, Spearman).
<<eval=FALSE>>=
data(cgps)
